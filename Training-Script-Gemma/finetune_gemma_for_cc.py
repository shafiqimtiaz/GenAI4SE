...
# To load tokenizer we need to set "local_files_only = True" and pass the path where model is stored instead of MODEL_ID.
def training(train_ds, valid_ds, max_epochs, batch_size):
    ...
    # To load local model we need to set "local_files_only = True" and pass the path where model is stored instead of MODEL_ID.
